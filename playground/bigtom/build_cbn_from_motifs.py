        ...
        for local_id, label in node_labels.items():
            if label:  # Skip empty labels
                ...
                if node_id not in nodes:
                    ...
                    if is_stance_node(label) and not is_stance_node(
                        nodes[node_id]["label"]
                    ):
                ...
                if src_local in local_to_global_id and tgt_local in local_to_global_id:
                    ...
                    if edge_id not in edges:
                        edges[edge_id] = {
                            "source": src_id,
                            "target": tgt_id,
                            "source_label": nodes[src_id][
                                "label"
                            ],  # Use the canonical label
                            "target_label": nodes[tgt_id][
                                "label"
                            ],  # Use the canonical label
                    ...
                    edges[edge_id]["strength"] = min(
                        0.7 + (edge_mention_count - 1) * 0.1, 1.0
                    )

    # Update node importance based on connectivity
    for node_id, node_data in nodes.items():
        connectivity = len(node_data["incoming_edges"]) + len(
        ...
        if node_data["evidence"]:
            avg_confidence = sum(e["confidence"] for e in node_data["evidence"]) / len(
                node_data["evidence"]
            )
...
def generate_mermaid_graph(nodes: Dict, edges: Dict) -> str:
    ...
    for node_id, node_data in nodes.items():
        ...
        if node_data.get("is_stance", False):
            ...
            lines.append(
                f"    style {node_id} fill:#ff9999,stroke:#333,stroke-width:3px"
            )
        elif len(node_data["incoming_edges"]) == 0:
            # Root nodes - rounded rectangle
            lines.append(f'    {node_id}["{label}"]')
            lines.append(
                f"    style {node_id} fill:#99ccff,stroke:#333,stroke-width:2px"
            )
        elif len(node_data["outgoing_edges"]) == 0:
            # Leaf nodes - double border
            lines.append(f'    {node_id}[["{label}"]]')
            lines.append(
                f"    style {node_id} fill:#ccffcc,stroke:#333,stroke-width:2px"
            )
        else:
...
def build_cbn_for_participant(
    participant_dir: Path, prolific_id: str
) -> Tuple[Dict, str]:
    """
    ...
    for node_id, node_data in nodes.items():
        ...
        if any(
            pattern in label.lower()
            for pattern in ["support for", "support", "belief", "opinion"]
        ):
            stance_score += 3.0
    ...
    for node_id, node_data, score in stance_candidates:
        if stance_count < 3 and (
            score >= 8.0 or stance_count == 0
        ):  # At least one stance node
            nodes[node_id]["is_stance"] = True
...
def process_single_participant(
    participant_info: Tuple[Path, str, Path],
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        if cbn:
            ...
            with open(md_file, "w") as f:
                ...
                f.write(
                    f"**Topic**: {cbn['graphs'][0]['graphData']['metadata']['topic']}\n"
                )
                f.write(
                    f"**Total Motifs**: {cbn['graphs'][0]['graphData']['metadata']['total_motifs']}\n"
                )
                f.write(
                    f"**Unique Nodes**: {cbn['graphs'][0]['graphData']['metadata']['unique_nodes']}\n"
                )
                f.write(
                    f"**Unique Edges**: {cbn['graphs'][0]['graphData']['metadata']['unique_edges']}\n\n"
                )
                ...
                f.write("## Node Types\n\n")
                f.write(
                    "- **Root Nodes** (Blue): Starting points with no incoming edges\n"
                )
                f.write(
                    "- **Intermediate Nodes** (White): Nodes with both incoming and outgoing edges\n"
                )
                f.write("- **Leaf Nodes** (Green): Endpoints with no outgoing edges\n")
                f.write(
                    "- **Stance Nodes** (Red): High-importance belief endpoints\n\n"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory containing participant motif folders (relative to this script)",
    )
    parser.add_argument(
        "--output_dir",
        default="results/participant_cbns",
        help="Directory to save individual CBN JSON files (relative to this script)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    # Fallback to old default if new default doesn't exist
    if not input_dir.exists():
        legacy_dir = base_dir.parent / "t2" / "results" / "motifs_from_transcripts"
        if legacy_dir.exists():
            print(f"Warning: {input_dir} not found. Falling back to {legacy_dir}")
    ...
    print(
        f"Using {'serial' if args.no_parallel else 'parallel'} processing with {args.workers if not args.no_parallel else 1} worker(s)"
    )
    ...
    participant_infos = [
        (participant_dir, participant_dir.name, output_dir)
    ...
    if args.no_parallel:
        ...
        for participant_info in tqdm(participant_infos, desc="Processing participants"):
            ...
            if result and result["success"]:
                successful += 1
                print(
                    f"✓ {result['prolific_id']}: {result['nodes']} nodes, {result['edges']} edges"
                )
            else:
                failed += 1
    ...
    if successful > 0:
        ...
        # Also save to main results directory for easy access (under current project area)
        main_results_dir = base_dir / "results"
        ...
        print(f"  ✓ Created combined CBN file: {combined_file}")
        print(f"  ✓ Created main CBN file: {main_cbn_file}")
