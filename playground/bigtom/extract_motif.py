...
from playground.bigtom.llm_utils import QwenLLM
...
def _build_llm_prompt(record: Dict[str, object]) -> str:
    ...
    instruction = (
        "Extract the participant's small reasoning motifs as 3-node graphs. Suppose you are the participant, what is the core reasoning process you used for scenario (which is clear in the transcript)? The goal is to capture the core reasoning process, so the name of the participant doesn't matter."
        "Nodes must be neutral, you can try abstracting the node labels to be more general and still reflecting your (as the participant) reasoning process; do not include opinion/effect words in node labels. "
...
def _build_bigtom_records_from_row(
    cols: List[str], row_idx: int
) -> List[Tuple[str, Dict[str, object]]]:
    """Build LLM-ready records for BigToM row scenarios (see and no-see).

    Each record uses the generic LLM extraction flow via context_qas (story, condition,
    belief Q/A, want Q/A, action Q/A).
    """
    out: List[Tuple[str, Dict[str, object]]] = []

    def safe(idx: int) -> str:
        return cols[idx].strip() if len(cols) > idx and cols[idx] is not None else ""
    ...
    # Answers for see scenario
    a_belief_see = safe(8)
    a_want_see = safe(9)
    a_action_see = safe(10)

    # Answers for no-see scenario
    a_belief_no = safe(11)
    a_want_no = safe(12)
    a_action_no = safe(13)
    ...
    def make_record(
        ...
        if story:
            qas.append(
                {"question_number": 1, "question": "Story context", "answer": story}
            )
        if condition_text:
            qas.append(
                {
                    ...
                    "answer": condition_text,
                }
            )
        if q_belief or a_belief:
            qas.append(
                {
                    "question_number": 3,
                    "question": q_belief or "Belief?",
                    "answer": a_belief,
                }
            )
        if q_want or a_want:
            qas.append(
                {
                    "question_number": 4,
                    "question": q_want or "Goal/Want?",
                    "answer": a_want,
                }
            )
        if q_action or a_action:
            qas.append(
                {
                    "question_number": 5,
                    "question": q_action or "Action?",
                    "answer": a_action,
                }
            )
        ...
        record = {
            "prolific_id": pid,
            "topic": "bigtom",
            "demographics": {"scenario": tag},
            "context_qas": qas,
        }
        return pid, record

    # Build see scenario record
    if see_cond and (a_belief_see or a_want_see or a_action_see):
        pid_see = f"bigtom_{row_idx:04d}_see"
        out.append(
            make_record(
                pid_see, see_cond, a_belief_see, a_want_see, a_action_see, "see"
            )
        )

    # Build no-see scenario record
    if no_cond and (a_belief_no or a_want_no or a_action_no):
        pid_no = f"bigtom_{row_idx:04d}_no"
        out.append(
            make_record(pid_no, no_cond, a_belief_no, a_want_no, a_action_no, "no")
        )

    # If neither scenario has usable content but there is a story, still create a stub
    if not out and story:
        pid = f"bigtom_{row_idx:04d}"
        out.append(
            (
                pid,
                {
...
def process_bigtom_csv(
    input_csv_path: str, output_dir: str, max_records: Optional[int] = None
) -> int:
    """Process a BigToM semicolon-delimited CSV into per-scenario motif outputs.

    Returns the number of participant folders written.
    """
    ensure_dir(output_dir)

    input_csv_path = str(CURRENT_DIR / input_csv_path)

    total_rows = _count_nonempty_lines(input_csv_path)
    # Each row typically yields up to 2 scenarios
    estimated_total = total_rows * 2

    written = 0
    seen_ids = set()

    print(f"Processing {estimated_total} rows...")
    max_records = max_records or estimated_total
    with open(input_csv_path, "r", encoding="utf-8") as f, tqdm(
        total=max_records, desc="Processing", position=0, leave=True
    ) as pbar:
        ...
        for row_idx, cols in enumerate(reader, start=1):
            ...
            # Build LLM-ready records per scenario from CSV
            records = _build_bigtom_records_from_row(cols, row_idx)
            for pid, record in records:
                if pid in seen_ids:
                    pbar.update(1)
                    continue
                seen_ids.add(pid)
                # Use existing LLM flow
                _, out_data = extract_motifs_from_record(record)
                write_per_prolific(output_dir, pid, out_data)
                written += 1
                pbar.update(1)
...
def main() -> None:
    ...
    input_path = str(CURRENT_DIR / args.input)
    output_dir = str(CURRENT_DIR / args.output_dir)

    # Branch on input type
    if args.input.lower().endswith(".csv"):
        print("Detected CSV input. Processing BigToM-format rows...")
        written = process_bigtom_csv(input_path, output_dir, args.max_records)
        print(f"Wrote {written} participant folders to {output_dir}")
        return

    # Default: JSONL transcripts path using LLM extraction
    print("Counting total records...")
    total_records = sum(1 for _ in read_jsonl(input_path, args.max_records))
    ...
    with tqdm(total=total_records, desc="Processing", position=0, leave=True) as pbar:
        for record in read_jsonl(input_path):
            ...
            write_per_prolific(output_dir, prolific_id, out_data)
