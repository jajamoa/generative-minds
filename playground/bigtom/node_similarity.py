...
def _initialize_embedding_model():
    ...
    # Silent fallback to word-based similarity
...
def _get_embedding(text: str) -> np.ndarray:
    ...
    if _embedding_type == "sentence_transformer":
        return _embedding_model.encode([text.strip()], show_progress_bar=False)[0]
        ...
        try:
            response = openai.embeddings.create(
                model=_embedding_model, input=text.strip()
            )
...
def find_similar_node(
    target_label: str, existing_nodes: Dict[str, Dict], threshold: float = 0.8
...
def get_consistent_node_id(
    label: str, existing_nodes: Dict[str, Dict], similarity_threshold: float = 0.8
) -> str:
    """
    ...
    similar_node_id, similarity = find_similar_node(
        label, existing_nodes, similarity_threshold
    )
