...
def process_single_question(cbn_agent, vqa, include_demographics, include_context, temperature, debug=False, swap_data=None, max_retries=3):
...
def evaluate_belief_inference(benchmark_path, cbn_path="sample_cbn.json", model="qwen-plus", temperature=0, include_demographics=True, include_context=True, max_workers=3, debug=False, swap_experiment=False, limit=None):
    ...
    cbn_agent = CBNAgent(model=model, cbn_path=cbn_path)
    ...
    print(f"Using CBN from: {cbn_path}")
    ...
    for difficulty, dataset in difficulty_datasets.items():
        ...
        if debug:
            ...
            for i, vqa in enumerate(dataset):
                ...
                result = process_single_question(cbn_agent, vqa, include_demographics, include_context, temperature, debug, swap_data, max_retries=5)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Theory of Mind belief inference benchmark with CBN Agent")
    parser.add_argument("--benchmark", type=str, required=True, 
                       help="Path to the benchmark JSONL file")
    parser.add_argument("--cbn", type=str, default="sample_cbn.json",
                       help="Path to the CBN definition file")
    ...
    try:
        ...
        results_filename = os.path.join(results_dir, f"cbn_results_{clean_model_name}.json")
