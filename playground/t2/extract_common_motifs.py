...
def normalize_motif_structure(motif: dict) -> Tuple[str, List[Tuple[str, str]]]:
    ...
    if pattern_type == MotifPattern.CHAIN:
        ...
        middle = [n for n in motif['nodes'] if in_degree[n] == 1 and out_degree[n] == 1][0]
        end = [n for n in motif['nodes'] if out_degree[n] == 0][0]
        
        return pattern_type, [('n1', 'n2'), ('n2', 'n3')], {start: 'n1', middle: 'n2', end: 'n3'}
...
def cluster_similar_motifs(all_motifs: Dict[str, List[dict]], 
                          similarity_threshold: float = 0.7) -> List[CommonMotif]:
    """
    ...
    for pattern_type, motif_list in pattern_groups.items():
        ...
        for motif_data in motif_list:
                ...
                for node_id in ['n1', 'n2', 'n3']:
                    if node_id in norm_labels and node_id in cluster.canonical_labels:
                        sim = compute_node_similarity(
                            norm_labels[node_id], 
                            cluster.canonical_labels[node_id]
                        )
                        total_sim += sim
            ...
            if best_cluster:
                best_cluster.add_instance(motif_data['transcript_id'], motif_data['motif'])
            else:
                # Create new cluster
                new_cluster = CommonMotif(pattern_type, norm_labels)
                new_cluster.add_instance(motif_data['transcript_id'], motif_data['motif'])
...
def generate_report(common_motifs: List[CommonMotif], output_dir: Path):
    ...
    for i, motif_data in enumerate(summary['motifs']):
        ...
        main_report.append(f"- [Details](./{motif_data['id']}.md) | [Diagram](./{motif_data['id']}.mmd)")
...
def main():
    ...
    motifs_dir = base_dir / "results" / "motifs_from_transcripts"
    output_dir = base_dir / "results" / "common_motifs_analysis"
    
    print("Loading motifs from transcripts...")
    all_motifs = load_all_motifs(motifs_dir)
    print(f"Loaded motifs from {len(all_motifs)} transcripts")
    
    print("\nClustering similar motifs...")
    common_motifs = cluster_similar_motifs(all_motifs, similarity_threshold=0.7)
    ...
    for i, motif in enumerate(common_motifs[:10]):
        print(f"{i+1}. {motif.pattern_type.title()} - "
              f"{motif.get_frequency()} instances in "
              f"{motif.get_transcript_count()} transcripts")
