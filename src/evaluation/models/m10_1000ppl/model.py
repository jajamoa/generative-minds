...
class M10_1000ppl(Census):
    """A model that generates opinions using OpenAI API and 1000ppl data."""
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        # Set up expert reflection data path from config or default
        self.expert_reflection_dir = config.expert_reflection_dir if hasattr(config, 'expert_reflection_dir') else os.path.join(os.path.dirname(__file__), "data", "expert_reflection")
    ...
    def _load_expert_reflection(self, agent_id: str) -> Optional[Dict[str, Any]]:
        """Load expert reflection data for a specific agent by matching filename with agent ID.
        
        Args:
            agent_id: The ID of the agent from agents_with_geo.json.
        """
        expert_reflection_file = os.path.join(self.expert_reflection_dir, f"{agent_id}.json")
        try:
            with open(expert_reflection_file, 'r', encoding='utf-8') as f:
                expert_reflection_data = json.load(f)
                print(f"DEBUG: Successfully loaded expert reflection for agent {agent_id}")
                return expert_reflection_data
        except Exception as e:
...
Based on the expert reflection:
{expert_reflection}
